...
* 2011/11/19       冯春松                  创 建
**********************************************************************/
static ssize_t  touchscreen_mode_store(struct device *dev,struct device_attribute *attr,const char *buf, int count)
{
  int ret=0;
  int mode=0;
...
static int touchscreen_gesture_ctrl_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
  int ret=0;
  if(buf==NULL)
  {
...
static DEVICE_ATTR(gesture_wakeup, 0664, touchscreen_gesture_wakeup_show, NULL);
static DEVICE_ATTR(gesture_ctrl, 0664, NULL, touchscreen_gesture_ctrl_store);
...
static int export_store(struct class *class, struct class_attribute *attr, const char *buf, int len)
{
  return 1;
}

static int unexport_store(struct class *class, struct class_attribute *attr,const char *buf, int len)
{
  return 1;
}
