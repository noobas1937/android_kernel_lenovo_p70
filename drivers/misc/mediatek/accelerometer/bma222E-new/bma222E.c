...
static struct acc_init_info gsensor_init_info = {
    .name = BMA222_DEV_NAME,
...
static int BMA222_ResetCalibration(struct i2c_client *client)
{
    ...
    len = sizeof(data.set_cust_req.custData) + sizeof(pCustData->resetCali);
...
static int BMA222_WriteCalibration(struct i2c_client *client, int dat[BMA222_AXES_NUM])
{
    ...
    len = sizeof(data.set_cust_req.custData) + sizeof(pCustData->setCali);
    SCP_sensorHub_req_send(&data, &len, 1);
#endif
...
static int BMA222_SetPowerMode(struct i2c_client *client, bool enable)
{
    ...
    if (res)
    {
        GSE_ERR("SCP_sensorHub_req_send failed!\n");
        return res;
    }
...
/*----------------------------------------------------------------------------*/
static int gsensor_set_batch(int flags, int64_t period_ns, int64_t timeout)
{
    int err = 0;
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(0 != (err = bma222_create_attr(&gsensor_init_info.platform_diver_addr->driver)))
	{
...
static int bma222_i2c_remove(struct i2c_client *client)
{
	...
	if(0 != (err = bma222_delete_attr(&gsensor_init_info.platform_diver_addr->driver)))
	{
...
static int __init bma222_init(void)
{
    ...
    acc_driver_add(&gsensor_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
